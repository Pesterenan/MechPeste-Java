...
public class decolagemOrbital {
	public static void main(String[] args) throws IOException, RPCException, InterruptedException, StreamException {
		Connection conexao = Connection.newInstance("Decolagem Orbital");
		...
		naveAtual.getControl().setThrottle(0f); // acelerar ao mÃ¡ximo
		...
		while (altitude.get() < 70500) {
			...
			while (tempoAteApoastro.get() - leadTime - (tempoQueima / 2.0) > 0) {
			}
