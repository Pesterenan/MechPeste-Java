...
public class Main {
  ...
  public static void main(String[] args) {
    try {
      ...
      Vector targetDirection =
          new Vector(activeVessel.position(orbitalRefVessel))
      ...
      while (Math.abs(lastYTargetPos) >= DISTANCE_LIMIT) {
        ...
        float forwardsError =
            calculateThrottle(
                ...
                SPEED_LIMIT);
        float sidewaysError =
            calculateThrottle(0, 10, currentXAxisSpeed, targetPosition.x, SPEED_LIMIT);
        float upwardsError =
            calculateThrottle(0, 10, currentZAxisSpeed, targetPosition.z, SPEED_LIMIT);
      ...
      Vector direcaoContrariaDockingPortAlvo =
      ...
      while (targetVessel != null) {
        ...
        // Calcular a aceleração para cada eixo no RCS:
        float forwardsError =
            calculateThrottle(5, 10, currentYAxisSpeed, targetPosition.y, SPEED_LIMIT);
        float sidewaysError =
            calculateThrottle(-1, 10, currentXAxisSpeed, targetPosition.x, SPEED_LIMIT);
        float upwardsError =
            calculateThrottle(-1, 10, currentZAxisSpeed, targetPosition.z, SPEED_LIMIT);
        control.setForward((float) forwardsError);
        control.setRight((float) sidewaysError);
        control.setUp((float) -upwardsError);
  ...
  private static float calculateThrottle(
      double minDistance,
      double maxDistance,
      double currentSpeed,
