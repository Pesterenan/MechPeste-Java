...
private int MAX_RADAR_LINES = 8;

private List<Drawing.Line> radarLines = new ArrayList<>();
private Vector[] directionAngles = { L90DEG, L60DEG, L30DEG, new Vector(0.0, 1.0, 0.0), R30DEG, R60DEG, R90DEG };
...
private void initializeParameters() {
	try {
		...
		pontoRefOrbital = naveAtual.getOrbit()
															 .getBody()
															 .getReferenceFrame();
	...
	} catch (RPCException | IOException exception) {
	}
}
...
private void lidar() throws RPCException, IOException, InterruptedException {
	while (!naveAtual.getControl()
		...
		steeringLine.setEnd(radarSourcePosition.sum(transformDirection(calculatedDirection.normalize()).multiply(50))
																					 .toTriplet());
		Thread.sleep(50);
	}
}
