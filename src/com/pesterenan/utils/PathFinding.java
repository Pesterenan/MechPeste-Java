...
private void initializeParameters() {
	try {
		...
		waypointsToReach = new ArrayList<Waypoint>();
		pathToTarget = new ArrayList<Vector>();
...
public void addWaypointsOnSameBody(String waypointName) throws RPCException {
	for (Waypoint waypoint : waypointManager.getWaypoints()) {
		boolean hasSameName = waypoint.getName()
																	.toLowerCase()
																	.contains(waypointName.toLowerCase());
		boolean isOnSameBody = waypoint.getBody()
																	 .equals(currentBody);
...
private Vector waypointPosOnSurface(Waypoint waypoint) throws RPCException {
	return new Vector(currentBody.surfacePosition(waypoint.getLatitude(), waypoint.getLongitude(), pontoRefOrbital));
}
...
public void buildPathToTarget(Vector targetPosition) throws IOException, RPCException, InterruptedException {
	...
	Vector currentRoverPos = transformSurfToOrb(new Vector(naveAtual.position(pontoRefSuperficie)).sum(roverHeight));
	...
	while (distanceToTarget > 100) {
		...
		index++;
		Vector directionToTarget = transformOrbToSurf(targetPosition).subtract(transformOrbToSurf(previousPoint))
																																 .normalize();
		Vector calculatedNextPoint =
						avoidObstacles(transformOrbToSurf(previousPoint), transformDirection(directionToTarget, true));
		Vector nextPoint = transformSurfToOrb(transformOrbToSurf(getPosOnSurface(calculatedNextPoint)).sum(roverHeight));
...
private Vector avoidObstacles(Vector currentPoint, Vector targetDirection) throws RPCException, IOException {
	...
	double centerDistance = raycastDistance(currentPoint, transformDirection(targetDirection, false),
					pontoRefSuperficie);
	double left15degDistance = raycastDistance(currentPoint, transformDirection(targetDirection.sum(L30DEG)
																																														 .normalize(), false),
					pontoRefSuperficie);
	double left30degDistance = raycastDistance(currentPoint, transformDirection(targetDirection.sum(L60DEG)
																																														 .normalize(), false),
					pontoRefSuperficie);
	double right15degDistance = raycastDistance(currentPoint, transformDirection(targetDirection.sum(R30DEG)
																																															.normalize(), false),
					pontoRefSuperficie);
	double right30degDistance = raycastDistance(currentPoint, transformDirection(targetDirection.sum(R60DEG)
																																															.normalize(), false),
					pontoRefSuperficie);
...
public double raycastDistance(Vector currentPoint, Vector targetDirection, SpaceCenter.ReferenceFrame reference) throws RPCException {
	...
	return Math.min(centroEspacial.raycastDistance(currentPoint.toTriplet(), targetDirection.toTriplet(), reference),
					searchDistance);
}
...
private Vector getPosOnSurface(Vector vector) throws RPCException {
	return new Vector(currentBody.surfacePosition(currentBody.latitudeAtPosition(vector.toTriplet(), pontoRefOrbital),
					currentBody.longitudeAtPosition(vector.toTriplet(), pontoRefOrbital), pontoRefOrbital));
}
